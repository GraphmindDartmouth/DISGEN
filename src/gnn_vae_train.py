...
from quick_test_train_util import train_step_gnn_vae, eval_step, train_data_gnn_vae
from quick_test_vae_model import GnnVae, BaselineGCN
import wandb
...
config_wandb = {}
# wandb.init(project="gnn_vae_multiview", config=config_wandb)
# close wandb
...
if os.path.isfile(model_save_path + explainer_model_name + "_saved.pt"):
    ...
    torch.save(explainer_model_name, model_save_path + explainer_model_name + "_saved.pt")
    print("Model saved!!")
...
gnn_input_dim = x_s
gnn_output_dim, vae_input_dim = 32, 32
vae_output_dim = 32

hidden_dim_1 = 32
hidden_dim_2 = 64
output_dim_encoder = 32
output_dim_decoder = 32

z_dim = 16
target_classes = y_s
sensitive_classes = 9
model_name = args.model
model = GnnVae(model_name, gnn_input_dim, gnn_output_dim, output_dim_encoder, output_dim_decoder, hidden_dim_1, hidden_dim_2, z_dim, target_classes, sensitive_classes)
print(model)

wandb.watch(model, log='all')

optimizer_1 = torch.optim.Adam(model.gnn.parameters(), lr=0.001)
...
train_data_gnn_vae(train_loader, val_loader, test_loader_small, test_loader_large, model, optimizer_1,
                   optimizer_2, device)
